...
3.  So first of all, let's try to create a new, "clean" environment.
    ...
    Let's activate it, so we can use it. Type:
...
5.  Let's deactivate this empty environment and delete it. To deactivate, type:
...
6.  Now let's try to create an environment *with* Python. Type:
    ...
    You can also try `which python`, depending on your platform.
    This should show you a path within the newly created environment,
    under your user, in a miniforge directory, rather than in a system location like
    `/bin` or `/usr/bin`.

7.  Now try running `python` to launch the interpreter and then `import numpy as np`. This
    should fail, but why?
...
3.  Now let's deactivate and delete this environment, as before using
    `conda deactivate` and `conda env remove -n bestpractices`.
